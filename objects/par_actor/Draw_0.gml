...
{ //
	...
	if (obj_game.hoverNode != noone and obj_game.hoverNode != map[gridX,gridY])
	{
		...
		{
			//purple inline
			draw_line_width_colour(current_node.x + (GRID_SIZE / 2),current_node.y + (GRID_SIZE / 2),current_node.parent_node.x + (GRID_SIZE / 2),current_node.parent_node.y + (GRID_SIZE / 2),2,c_purple,c_purple);
...
{
	draw_healthbar(x,y+GRID_SIZE+1,x+GRID_SIZE,y+GRID_SIZE+2,current_hit_points/max_hit_points * 100,c_black,c_red,c_yellow,1,true,true);
}