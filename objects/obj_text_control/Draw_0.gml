...
if (global.hud_on == true)
{
	...
	if IsChatterbox(chatterbox) and text != undefined //nothing is drawn with no text
	{
		...
		if (right_char_index == old_right_char_index)
		{
			...
			draw_sprite_ext(spr_characters, right_char_index, margin_char + right_slide_in, room_height -75,size[me],size[me],0,colour[me],1);
		}
		...
		if (left_char_index == old_left_char_index)
		{
			...
			draw_sprite_ext(spr_characters, left_char_index, room_width - margin_char - left_slide_in, room_height -75,size[!me],size[!me],0,colour[!me],1);
		}
		else if (left_char_index != old_left_char_index and finish_left_slide == true)
		{
			left_slide_in = 20;
			finish_left_slide = false;
			draw_sprite_ext(spr_characters, left_char_index, room_width - margin_char - left_slide_in, room_height -75,size[!me],size[!me],0,colour[!me],1);
		}
		else
		{
