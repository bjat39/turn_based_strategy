	...
	while(ds_priority_size(open) > 0){
		...
		for(ii = 0; ii < ds_list_size(current_node.neighbours); ii ++)
		{
			...
			if (ds_list_find_index(closed, curr_neighbour) < 0)
			{
				...
				curr_neighbour.cost + current_node.attack_G <= range //movement range
				or ((!curr_neighbour.passable or curr_neighbour.occupant != noone)
				and 1 + current_node.attack_G <= attack_range))
				...
				else if ((!curr_neighbour.passable or !curr_neighbour.occupant == noone) and 1 + current_node.attack_G <= full_attack_range)// and current_node.G >= range)
				{
					//only calculate new G for neighbour if it hasn't already been calculaated
					if (ds_priority_find_priority(open,curr_neighbour) == 0 or ds_priority_find_priority(open,curr_neighbour) == undefined) 
					{
